...
function isLeapYear(year) {
  ...
  if (typeof year === "number") {
    return { year: `${year}`, isLeap: !!(year / 4) };
  } else {
  ...
  if (date.getTime()) {
    if (typeof date.getDate() === "number") {
      ...
      return {
        ...
        addedDays: days,
        resultDate: newDate.toISOString(),
      };
    } else {
...
function getDayOfWeek(date) {
  ...
  if (date.getTime()) {
    let day = date.getDay();
    return { inputDate: date.toISOString(), dayOfWeek: daysOfWeek[day] };
  } else {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
}
...
function getDaysInMonth(date) {
  ...
  if (date.getTime()) {
    ...
    let newDate = new Date(2023, 6, 1);
    let nowDate = new Date();
    let diff = nowDate.getDate() - (newDate.getDate() - 1);
    return { inputDate: date.toISOString(), daysInMonth: diff };
  } else {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
}
